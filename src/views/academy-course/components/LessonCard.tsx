/**
 * NOT IN USE. REMOVE THIS FILE AND UPDATE REFERENCE TO PreviewLessonCard
 */
...
const LessonCard: FC<Props> = ({ lesson, courseId }) => {
  return (
    <ConditionalWrapper
      condition={lesson.status != "scheduled"}
      wrapper={(children: ReactElement) => <Link to={`/academy/course/${courseId}/lesson/${lesson.id}`}>{children}</Link>}
    >
      <div className="w-68">
        ...
        <div className="flex mt-2">
          <div className="ml-2">
            ...
            <div className="text-xs text-gray-600">
              ...
              <p>{dayjs(lesson.created_at.seconds * 1000).fromNow()} {/*&middot; {lesson.count_views} views*/}</p>
            </div>
          </div>
        </div>
      </div>
