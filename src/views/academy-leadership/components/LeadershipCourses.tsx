...
import LeadershipCourseCard from './LeadershipCourseCard'
import useLeadershipCourses from '@/hooks/useLeadershipCourses'

const LeadershipAcademyCourses = () => {
  ...
  return (
    <div>
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-y-10 gap-x-2">
        ...
        {courses.map((course) => (
          <LeadershipCourseCard key={course.id} course={course} />
        ))}
      </div>
    </div>
  )
...
export default LeadershipAcademyCourses
